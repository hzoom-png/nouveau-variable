...
import { getN2Rate, N2_TIERS, SUBSCRIPTION, N1_RATE } from '@/lib/constants'
  ...
  return (
    <div style={{ maxWidth: '900px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '32px' }}>
        {kpis.map(k => (
          <div key={k.label} style={{ background: 'white', borderRadius: '14px', padding: '20px', boxShadow: '0 1px 6px rgba(67,105,90,0.07)' }}>
      ...
      <div style={{ background: 'white', borderRadius: '14px', padding: '24px', marginBottom: '24px', boxShadow: '0 1px 6px rgba(67,105,90,0.07)' }}>
        <div style={{ fontFamily: 'var(--font-jost, Jost, sans-serif)', fontWeight: 700, fontSize: '16px', color: 'var(--text)', marginBottom: '6px' }}>
          Taux de commission N2 — Palier actuel : <span style={{ color: '#43695A' }}>{n2Rate}%</span>
        </div>
        {nextTier && (
          <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '16px' }}>
            Encore {nextTier.min - n1} filleuls N1 pour atteindre {nextTier.rate}%
          </div>
        )}
        <div style={{ display: 'flex', gap: '6px', marginBottom: '8px' }}>
          {N2_TIERS.map(tier => {
            const reached = n1 >= tier.min
            return (
              <div key={tier.min} style={{ flex: 1 }}>
                <div style={{
                  height: '10px', borderRadius: '6px',
                  background: reached ? '#43695A' : 'var(--border)',
                  marginBottom: '4px',
                }} />
                <div style={{ fontSize: '11px', color: reached ? '#43695A' : 'var(--muted)', textAlign: 'center', fontWeight: reached ? 700 : 400 }}>
                  {tier.rate}%
                </div>
                <div style={{ fontSize: '10px', color: 'var(--light)', textAlign: 'center' }}>
                  {tier.min}+ N1
                </div>
              </div>
            )

...
export async function GET() {
  ...
  const [{ data: sent }, { data: received }] = await Promise.all([
    service
      ...
      .select('*, recipient:profiles!meeting_requests_recipient_id_fkey(first_name, last_name, avatar_url, role_title, slug)')
      .eq('requester_id', user.id)
      .order('created_at', { ascending: false }),
    service
      .from('meeting_requests')
      .select('*, requester:profiles!meeting_requests_requester_id_fkey(first_name, last_name, avatar_url, role_title, slug)')
...
export async function POST(request: Request) {
  ...
  const { recipient_id, meeting_type, proposed_slots, preferred_days, preferred_moments, message, availability_note } = parsed.data
  ...
  const isNewType = (VALID_NEW_TYPES as readonly string[]).includes(meeting_type)
  if (!mt && !isNewType) {
  ...
  const { data: recipientProfile } = await service
    .from('profiles')
    .select('is_active, is_manually_activated, is_founder, email, first_name, phone')
    .eq('id', recipient_id)
    .single()

  if (!recipientProfile) return NextResponse.json({ error: 'Profil introuvable' }, { status: 400 })
  if (!recipientProfile.is_active && !recipientProfile.is_manually_activated && !recipientProfile.is_founder) {
    return NextResponse.json({ error: "Ce membre n'est pas disponible pour les rencontres" }, { status: 400 })
  }
  ...
  if (existing) {
    return NextResponse.json({ error: 'Une demande est déjà en attente avec ce membre' }, { status: 400 })
  }

  const { data: requesterProfile } = await service
    .from('profiles')
    .select('points_balance, first_name, last_name, phone')
    .eq('id', user.id)
    .single()
  ...
  try {
    if (recipientProfile.phone) {
      const senderName = `${requesterProfile.first_name ?? ''} ${requesterProfile.last_name ?? ''}`.trim()
      await sendSMS(recipientProfile.phone, SMS_TEMPLATES.newMeetingRequest(senderName))
    }
  } catch (e) {
    console.error('[meetings] Erreur SMS:', e instanceof Error ? e.message : e)
  }

  // Email to recipient
  try {
    if (recipientProfile.email && process.env.BREVO_API_KEY) {
      const safeFirst = escHtml(requesterProfile.first_name ?? '')
      const safeLast  = escHtml(requesterProfile.last_name  ?? '')
      const safeLabel = escHtml(mt?.label ?? meeting_type)

...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await service.from('points_transactions').insert({
    ...
    transaction_type: 'meeting_request_debit', // refund
    ...
    if (requesterProfile.email && process.env.BREVO_API_KEY) {
      await fetch('https://api.brevo.com/v3/smtp/email', {
        ...
        body: JSON.stringify({
          ...
          htmlContent: `<h2>Demande déclinée</h2><p>Ta demande de rencontre a été déclinée. Tes ${refundAmount} points ont été remboursés.</p><p><a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard/members">Découvrir d'autres membres →</a></p>`,
        }),
      })
    }
  } catch (e) {
